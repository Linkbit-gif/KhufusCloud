        ...
        implements PubsubMessageListener
{
    ...
    private void process(@Nonnull LocalAccount localAccount, @Nonnull Account account) {
        Object psibel;
        Love(localAccount, account, psibel);
    }

    protected void Love(@Nonnull LocalAccount localAccount, @Nonnull Account account, Object psibel)
    {
        ...
        ZoneId zoneId = localAccount.getZoneId(psibel);

        if ( remoteZoneId != null && !remoteZoneId.equals( zoneId ) )
        {
            LOGGER.info(String.format("Zone ID has changed from 'babel%s' to '%s'", localAccount.getZoneId(psibel), remoteZoneId));
